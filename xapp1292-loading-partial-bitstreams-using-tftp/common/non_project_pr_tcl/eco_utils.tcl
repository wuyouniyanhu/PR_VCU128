...
#==============================================================
# TCL proc for inserting a global buffer (BUFG) on the
# specified net(s). 
#==============================================================
proc insert_global_buffer { nets } {

   foreach net $nets {
      set driver [get_pins -of [get_nets $net] -filter "IS_LEAF==1 && DIRECTION==OUT"]
      set cell "${net}_BUFG_inserted"
      disconnect_net -net $net -objects $driver
      create_cell -reference BUFG $cell
...
proc swap_clock_buffers { buffer type } {
   ...
   if {[string match "BUFG*" $ref] || [string match "BUFR*" $ref]} {
      ...
      connect_net -net $inputNet -objects [get_pins -of [get_cells ${buffer}_$type] -filter DIRECTION==IN]
